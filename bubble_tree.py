...
##TO DO: RECOGNIZE AND SKIP HASHED OUT FIRST ROW
...
requireparser.add_argument('-t', '--tree', help='Tree file', required=True)
...
#uncomment below for hashed out first row of file
#biom = pd.read_csv(args.input, sep="\t", skiprows=1)
...
def gennormbubble():
	...
	normdf = pd.DataFrame()
	samps = list(biom['OTUID'][1:]) #save first column to append to norm dataframe
	...
	normdf.insert(loc=0, column="OTUID", value=samps)
	reorderbiom(normdf)
...
def gennormheat():
	...
	samps = list(biom['OTUID'][1:]) #save first column to append to norm dataframe
	for i in range(1, len(biom)):
		normdata = biom.iloc[i][1:-2].apply(lambda x: np.log(x+1))
		normdf = normdf.append(normdata)
		#normdata = biom.iloc[i][1:-2].apply(np.log(i)).fillna(0)
...
def reorderbiom(normdf):
	...
	subset = normdf.loc[normdf['OTUID'].isin(leaves)].set_index('OTUID') #pull nodes from biom and reorder by leaves in tree
...
def heat(final):
	...
	sns.heatmap(final, mask=False, cmap="Accent", xticklabels=True, yticklabels=True, square=True)
	#save
	#plt.show() #turn on for testing only
